...
**Estado real:** el Core usa esos headers/su propio JWT vía Spring Security (`@PreAuthorize` por rol en cada controller). `ms-notas` y `ms-asistencias` no tienen Spring Security propio, pero desde 2026-07-09 leen `X-User-Roles` manualmente vía `RoleGuard.exigirRol` (`backend/commons/.../util/RoleGuard.java`, invocado al inicio de cada método de `CalificacionParcialController`/`AsistenciaController`): ADMIN/ADMINISTRATIVO/PROFESOR pueden crear/listar/actualizar, solo ADMIN/ADMINISTRATIVO pueden eliminar (403 si no matchea). Ver `.remember/PENDIENTES.md` ítem 7.
