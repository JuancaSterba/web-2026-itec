...
sequenceDiagram
    ...
    alt DNI ya existe (otro rol sobre la misma persona)
        ...
        Service->>Service: enabled = false para ALUMNO<br/>(PROFESOR/ADMIN/ADMINISTRATIVO nacen enabled=true)
...
- **Cuentas deshabilitadas por defecto:** `ALUMNO` se crea con `enabled=false` ya que actualmente no operan el sistema. Los roles `PROFESOR`, `ADMIN` y `ADMINISTRATIVO` nacen con `enabled=true` permitiéndoles ingresar al sistema inmediatamente (refactor aplicado para profesores el 2026-07-12).
