...
> **Estado real (2026-07-12):** el cálculo del 70% y la comunicación MS_Asistencias→Core (pasos del `alt Pierde la Regularidad`) **no están implementados** — es el diseño requerido, no el actual. Hoy el flujo real es: `TomarAsistenciaDialog` (frontend) arma un lote de `{cursadaId, fecha, estado}` por comisión y hace `POST/PUT /api/asistencias` directo contra `ms-asistencias` a través del Gateway (`saveAsistenciasMasivas`, upsert por `cursadaId+fecha`) — sin ningún cálculo de porcentaje ni notificación al Core. Ver `.remember/PENDIENTES.md` (deuda técnica).
