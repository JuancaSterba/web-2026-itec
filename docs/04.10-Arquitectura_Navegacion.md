...
> **Lectura Previa Recomendada:** [04.00-Arquitectura_Funcional.md](./04.00-Arquitectura_Funcional.md) (Define el modelo de dominio y las entidades que sustentan esta navegación).
...
* 🧑‍🏫 **Cuerpo Docente**
  * Profesores / Buscador
* 🎓 **Mis Comisiones** *(Acceso exclusivo y directo para el rol Profesor)*
...
5. **Panel Principal de la Comisión:** (`/dashboard/comisiones/:id_comision`)
   ...
   * *Navegación interna (Pestañas/Tabs):*
     * ℹ️ **Docentes / Info General:** Horarios y cuerpo docente asignado *(Oculto para rol Profesor)*.
     * 👥 **Alumnos:** Nómina de inscritos *(Solo-lectura para Profesor)*.
...
> **Estado (2026-07-12):** La migración a la estructura de navegación jerárquica ya se completó. Asimismo, se implementaron las restricciones de acceso por rol, garantizando que el Dashboard y las vistas de Comisiones se adapten de forma reactiva (ej. tabs read-only u ocultas) según si el usuario es `ADMIN` o `PROFESOR`.
