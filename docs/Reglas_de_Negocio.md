...
- **`DELETE /api/core/alumnos/{id}` y `/profesores/{id}` ahora también deshabilitan el `Usuario`** (`UserLookupPort.deshabilitar(userId)`, mismo patrón de puerto), no solo la baja lógica de `Alumno`/`Profesor`. Validado: habilitando manualmente una cuenta (simulando el caso futuro), el login funcionaba antes del DELETE y quedaba rechazado (403 `ACCOUNT_DISABLED`) después.
