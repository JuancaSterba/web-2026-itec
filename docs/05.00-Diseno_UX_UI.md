...
* **Menú Lateral (Sidebar):** 
  * Actúa como el mapa global. Su contenido es estrictamente reactivo al rol del usuario (ej. un Profesor solo ve "Mis Comisiones", mientras un Administrador ve todo el ecosistema). 
...
* Debajo del título, aparecen las pestañas: `[Información General]` `[Alumnos Inscritos]` `[Asistencias]` `[Calificaciones]`. Estas reaccionan al rol (gatekeeping visual): un profesor no ve la pestaña general ni puede editar alumnos.
...
### Escenario Alternativo: El Profesor opera su clase.
1. **Acceso Directo:** El menú lateral le ofrece únicamente `Mis Comisiones`. Al entrar, ve directamente sus propias comisiones activas (filtrado de seguridad por backend).
2. **Nodo Final Simplificado:** Al entrar a una comisión, el sistema esconde la pestaña "Info General" y renderiza la de "Alumnos" en modo *solo-lectura*. Todo el flujo de usuario lo empuja intuitivamente a sus tareas reales: cargar notas y asistencias.
